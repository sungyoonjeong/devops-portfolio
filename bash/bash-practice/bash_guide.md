...
### 🔍 명령어 쪼개어 보기
* **`cpu=$( ... )`**
  * 괄호 `()` 안의 명령어들을 실행한 최종 결과를 `cpu`라는 변수에 저장합니다.
* **`top -bn1`**
  * 시스템 작업 관리자인 `top` 명령어를 실행합니다.
  * `-b` (Batch mode): 데이터를 가공할 수 있도록 실시간 화면 갱신을 끄고 text 스트림 형태로 출력합니다.
  * `-n1` (Iteration): 딱 1번만 화면을 출력하고 명령어를 즉시 종료합니다.
* **`|` (파이프라인)**
  * 앞 명령어의 출력 결과를 뒤 명령어의 입력으로 전달합니다.
* **`grep "Cpu(s)"`**
  * `top` 전체 결과 중에서 `"Cpu(s)"` 문자가 포함된 행만 필터링합니다.
* **`awk '{print $2}'`**
  * 필터링된 행에서 **두 번째 필드(공백 기준 분할 단어)**를 추출합니다. 일반적으로 Linux `top` 결과의 2번째 필드는 '사용자 프로세스가 사용하는 CPU 사용률(us)'입니다.
* **`echo "CPU: $cpu"`**
  * 변수에 담긴 수치를 화면에 표시합니다. (출력 예: `CPU: 12.5`)
...
### 🔍 명령어 쪼개어 보기
* **`df /`**
  * 최상위 루트 디렉토리(`/`)가 위치한 디스크의 용량 및 사용량 정보를 조회합니다.
* **`tail -1`**
  * `df` 명령어 결과는 첫 줄이 제목(Header), 둘 줄이 실제 데이터입니다. 이 중 **마지막 1줄(실제 데이터 행)**만 가져옵니다.
* **`awk '{print $5}'`**
  * 가져온 데이터 행에서 **다섯 번째 필드**를 추출합니다. `df` 표준 출력의 5번째 열은 디스크의 **사용 퍼센트(Use%)**를 의미합니다.
* **`echo "디스크: $disk"`**
  * 변수에 담긴 수치를 화면에 표시합니다. (출력 예: `디스크: 45%`)
...
명령어의 실행 결과를 변수에 담는 방식을 **명령어 치환(Command Substitution)**이라고 합니다. 구식 백틱 대신 **`$()` 방식을 권장하는 이유**는 다음과 같습니다.

1. **시각적 가독성**
   * 백틱(\`)은 작은따옴표(')나 쌍따옴표(")와 모양이 비슷하여 오타를 찾기 어렵지만, `$()`는 괄호 형태라 명확히 구분됩니다.
2. **중첩(Nesting) 사용의 편리함**
   * 명령어 안에 또 다른 명령어를 넣을 때 백틱은 내부 백틱마다 역슬래시(`\``) 이스케이프 처리를 해야 해서 코드가 지저분해집니다.
   * 반면 `$()`는 `$(cat $(ls))` 처럼 가독성을 유지하며 얼마든지 겹쳐 쓸 수 있습니다.
---
