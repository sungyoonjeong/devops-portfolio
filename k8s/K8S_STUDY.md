...
### 실습 기록 (7/7) — Pod·Deployment 손으로 만져보기

3노드(마스터1+워커2) 클러스터에서 실제로 실행한 기록. 클러스터가 이미 3노드라 파드가 매번 다른 노드로 흩어지는 게 그대로 보인다.
...
`describe`의 Events를 보면 상태 흐름이 그대로 찍힌다: `Scheduled → Pulling → Pulled(19.9초 걸림, 이미지 161MB) → Created → Started`. 5장 "Pod 동작 flow"에서 배운 그 단계다.

**클러스터 안에서 curl 확인**

WSL에서 파드 IP로 바로 curl은 안 되니(전에 확인한 그대로) 노드 안에서 확인:
...
둘 다 각 웹서버의 기본 환영 페이지 HTML이 그대로 반환됨.

**index.html 수정 — 파일 수정이 즉시 반영되는지 확인**
...
컨테이너 안 파일을 직접 고치면 재시작 없이 바로 반영된다 — D5에서 nginx html 고치고 새로고침만 하면 되던 것과 같은 원리(파일은 요청마다 새로 읽힘). 다만 이 방식은 파드가 재생성되면 사라지는 임시 수정이라는 것도 같이 기억할 것 (10장 ConfigMap이 이 문제의 정식 해법).
...
명령형(`run`)과 결과는 똑같은 Pod지만, 파일이 남아 재현·리뷰가 가능하다는 차이는 여기서도 그대로.
...
3개가 마스터·워커1·워커2에 하나씩 — Deployment도 결국 scheduler가 분산 배치한다.

self-healing 확인 — 파드 하나를 직접 지워본다:
...
지운 지 몇 초 만에 이름이 다른 새 파드가 자동으로 채워진다 — ReplicaSet이 "3개 유지"를 계속 감시하고 있다는 증거(6장에서 개념으로 배울 내용을 먼저 실물로 봄).

`kubectl edit deployment web-deploy`로 replicas를 3→5로 고쳐도 되고, 같은 결과를 `scale`로도 낼 수 있다:
...
5개로 늘어남. `edit`이든 `scale`이든 결국 같은 `spec.replicas` 필드를 바꾸는 것 — 방법만 다르고 결과는 동일.
...
독립 파드(nginx-pod·httpd-pod·yaml-pod)는 지운 대로 사라졌는데, Deployment 소속 파드 5개는 삭제하자마자 전부 새 이름으로 재생성됐다. **`delete pod`는 파드만 지우는 것이고, "5개를 유지하라"는 선언(Deployment) 자체는 그대로 살아있기 때문** — 지우는 대상을 파드가 아니라 컨트롤러로 바꿔야 진짜로 없어진다:

```bash
kubectl delete deployment web-deploy
kubectl get pods -A | grep -v kube-system   # 출력 없음 = default 네임스페이스 완전히 빔
```

이 실습의 핵심 결론: **"파드를 지운다"와 "그 파드를 관리하는 컨트롤러를 지운다"는 다른 일이다.** 6장에서 Deployment·ReplicaSet 개념을 배우기 전에 몸으로 먼저 겪은 셈.
...
minikube는 노드가 1개라 실습 의미가 약하다. 명령만 익혀두고 개념 위주로.
