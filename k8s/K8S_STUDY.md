...
### apiserver로 오는 모든 요청이 통과하는 3단계

kubectl이든 다른 부품이든, apiserver에 도달한 요청은 순서대로 이 파이프라인을 통과한다:

```
요청 ──► ① 인증(Authentication) ──► ② 인가(Authorization) ──► ③ Admission Controller ──► etcd 기록
         "너 누구야"                "그 동작을 해도 되나"         "요청 자체가 정책에 맞나(수정도 가능)"
```

- **인증**: 클라이언트가 누구인지 확인. kubectl은 `~/.kube/config`의 인증서·토큰으로, 파드 내부에서 apiserver를 호출할 땐 자동 마운트되는 ServiceAccount 토큰으로 신원을 증명한다.
- **인가**: 그 신원이 "이 네임스페이스에서 이 동작(get/create/delete 등)을 해도 되는가" 판정. 표준 방식이 **RBAC**(Role-Based Access Control) — Role/ClusterRole(권한 목록)과 RoleBinding/ClusterRoleBinding(누구에게 줄지)의 조합. 입문 범위 밖이라 CKA 준비(8월)에서 핵심으로 다룬다.
- **Admission Controller**: 인가까지 통과한 요청을 마지막으로 검사·수정하는 플러그인 체인. 예: `LimitRanger`(requests/limits 기본값 주입), `ResourceQuota`(네임스페이스 자원 상한 검사). 여기서 거부되면 인가를 통과해도 최종 실패한다.

세 단계 다 통과해야 etcd에 기록되고, 그제야 controller-manager·scheduler가 반응을 시작한다(파드 뜨기까지의 흐름은 위 "파드 하나가 뜨기까지" 참고).
...
### 5-8. Pod 종료 과정 — Graceful Shutdown

`kubectl delete pod`를 치면 즉시 죽는 게 아니라 정해진 순서를 밟는다:

```
1. Pod 상태가 Terminating으로 바뀜, Service의 Endpoints에서 즉시 제외(신규 트래픽 차단)
2. preStop 훅이 있으면 먼저 실행
3. 컨테이너에 SIGTERM 전달 — "정리하고 스스로 끝내라"는 신호
4. terminationGracePeriodSeconds(기본 30초) 안에 안 끝나면 SIGKILL로 강제 종료
```

```yaml
spec:
  terminationGracePeriodSeconds: 60   # 정리 시간이 더 필요한 앱(진행 중 요청 마무리 등)이면 늘림
  containers:
  - name: app
    image: myapp
    lifecycle:
      preStop:
        exec:
          command: ["sh", "-c", "sleep 5"]   # LB에서 빠질 시간을 벌어주는 흔한 패턴
```

**왜 중요한가**: SIGTERM을 무시하는 앱은 30초를 꽉 채우고 강제 종료되면서 처리 중이던 요청이 끊긴다. PF2 Go 서버라면 `signal.Notify`로 SIGTERM을 받아 `http.Server.Shutdown()`(진행 중인 요청은 끝내고 새 요청만 거부)을 부르는 게 정석 — 이게 안 돼 있으면 배포할 때마다(롤링이든 카나리든) 순간적으로 502가 튄다. 캡스톤의 페일오버 MTTR 측정에서 "종료가 얼마나 깔끔한가"가 그대로 수치에 반영되는 지점이다.
...
`minReadySeconds`도 같이 알아둘 필드 — 파드가 Ready 판정을 받은 뒤 이 시간만큼 더 문제없이 떠 있어야 "정말 사용 가능"으로 카운트한다(기본 0). 이 값을 올리면 배포 하나하나가 신중해지는 대신 전체 롤아웃 속도는 느려진다 — 즉시 죽는 게 아니라 몇 초 뒤 죽는 버그를 롤아웃 도중 잡아내고 싶을 때 쓴다.

### Pod Disruption Budget (PDB) — 자발적 중단으로부터 최소 가용량 보호

노드 드레인(`kubectl drain`, 유지보수·오토스케일러 축소)처럼 **사람이나 시스템이 의도적으로** 파드를 없앨 때, 한 번에 너무 많이 비면 서비스가 끊긴다. PDB는 "이 아래로는 동시에 못 내려간다"는 하한선을 걸어둔다.
...
metadata:
  name: web-pdb
spec:
  minAvailable: 2        # 항상 최소 2개는 떠 있어야 함 (또는 maxUnavailable: 1)
  selector:
    matchLabels:
      app: web
```

노드 드레인이 이 조건을 못 지키면 kubectl은 그 파드를 건너뛰고 대기한다 — 강제로 밀어붙이지 않는 한 서비스가 끊기지 않는다. 장애(노드 다운 같은 **비자발적** 중단)는 PDB가 막을 수 없다는 것도 같이 기억(그건 replicas·안티어피니티의 역할). 캡스톤의 페일오버 실습에서 "왜 배포 중엔 안 끊기는데 노드를 강제로 뽑으면 끊기는가"를 설명할 근거가 이것.

### Canary · Blue-Green 배포 — Rolling Update로 못 하는 것

*(강의 원 커리큘럼엔 없는 내용이지만, 실무·포트폴리오·면접에서 "배포 전략"을 물으면 Rolling Update 다음으로 반드시 나오는 주제라 여기서 보완한다. PF1·캡스톤의 ArgoCD 단계에서 그대로 실습할 예정.)*

RollingUpdate는 결국 **전체를 순차적으로 새 버전으로 갈아치우는** 전략이다. 문제는 "10%만 잠깐 새 버전에 태워보고, 이상 없으면 늘리고, 이상 있으면 즉시 되돌리기"가 안 된다는 것 — maxSurge/maxUnavailable을 아무리 조절해도 결국 100%까지 진행되는 한 방향 전략이다. **트래픽 비율을 사람이 통제하며 점진적으로 늘리는** 것은 별도 패턴이 필요하다.

**Canary(카나리)** — 신버전에 트래픽 일부만 흘려보내 관찰 후 점진 확대:
...
K8s Deployment·Service 자체엔 "트래픽의 몇 %"라는 개념이 없다 — Service는 selector에 맞는 파드들에게 **균등 분배**만 한다. 그래서 카나리는 이 성질을 역이용해서 만든다:

```yaml
# stable: replicas 9, canary: replicas 1 → 파드 수 비율 = 대략적인 트래픽 비율(9:1)
apiVersion: apps/v1
kind: Deployment
metadata:
  name: web-stable
spec:
  ...
  template:
    metadata:
      labels: { app: web, track: stable }   # Service selector엔 track 없이 app: web만
    spec:
...
spec:
  selector:
    app: web        # ★ track은 안 봄 — stable·canary 파드 둘 다 뒤에 붙는다
  ports: [{ port: 80 }]
```

핵심은 Service의 selector가 `app: web`까지만 보고 `track`은 안 본다는 것 — 그래서 두 Deployment의 파드가 **같은 Service 뒤에 같이 편입**되고, kube-proxy는 파드 단위로 균등 분배하니 결과적으로 9:1 비율이 트래픽 비율이 된다(9장 Label의 "selector는 label로 느슨하게 연결된다"가 여기서 실전 활용된다). 관찰 후 문제없으면 canary replicas를 늘리고 stable을 줄이며 100%까지 이관, 문제 있으면 canary replicas=0으로 즉시 트래픽 완전 차단(롤백이 이미지 재배포보다 훨씬 빠르다).

이 방식의 한계: 비율이 "파드 개수의 정수비"로만 정해져서 세밀한 %(예: 1%)가 어렵고, 세션이 파드마다 랜덤 분배라 sticky session이 안 된다. 더 정교하게 하려면:

- **Ingress 기반 weighted canary**: nginx ingress controller는 canary 전용 annotation을 지원한다. 별도 Ingress를 canary로 선언하고 가중치를 %로 직접 지정:
  ```yaml
  metadata:
    annotations:
      nginx.ingress.kubernetes.io/canary: "true"
      nginx.ingress.kubernetes.io/canary-weight: "10"   # 정확히 10%
  ```
- **Argo Rollouts**(진짜 실무 표준): Deployment를 대체하는 CRD로, canary 단계·대기시간·Prometheus 메트릭 기반 자동 승격/자동 롤백까지 선언형으로 정의한다. 사람이 %를 수동으로 안 올려도 지표가 나쁘면 스스로 되돌린다. **CI/CD 블록(7/18 ArgoCD)에서 이어서 실습** — 지금은 "Service selector 트릭으로 카나리의 원리를 직접 만들어봤다"는 게 이번 학습의 목적이고, 실전 도구는 그다음.

**Blue-Green(블루그린)** — 카나리가 "비율을 서서히"라면, 블루그린은 "완전히 새 환경을 다 띄워놓고 트래픽을 한 번에 통째로 전환":

```
Blue(v1, 현재 서비스 중)   ──┐
                            (Service selector를 blue → green으로 원자적 전환)
Green(v2, 미리 다 띄워둠)  ──┘
```

```bash
kubectl apply -f web-green.yaml                     # green 배포, 아직 트래픽 없음(다른 label)
# green이 준비됐는지 충분히 검증한 뒤
kubectl patch service web-svc -p '{"spec":{"selector":{"track":"green"}}}'   # 트래픽 전환은 이 한 줄
# 문제 생기면 selector를 blue로 되돌리면 즉시 원복
```

카나리와의 차이: 카나리는 **일부 사용자만 먼저 신버전을 겪고 점진 확대**(위험 분산), 블루그린은 **아무도 안 겪다가 한 번에 전부 전환**(검증 후 일괄 전환, 되돌리기가 selector 한 줄로 즉각적). 블루그린은 새 버전만큼의 자원을 이중으로 잠깐 띄워야 해서 비용이 더 들고, 카나리는 실제 사용자 트래픽으로 관찰하다가 문제를 뒤늦게 발견할 수 있다는 게 트레이드오프. 부록 C에 Recreate까지 포함해 넷을 표로 비교했다.
...
용도: GPU 노드, SSD 노드, 특정 존 등. 더 정교한 제어는 nodeAffinity(선호도 — "가능하면 이런 노드", 강제가 아닌 가중치 조절도 가능)와 taint/toleration인데 입문 범위 밖 — CKA에서 다룬다. 개념만 미리 짚으면: **taint = 노드에 붙이는 거부 표식**("나는 아무 파드나 안 받는다"), **toleration = 파드가 그 거부를 견딜 수 있다는 허용 표시**. nodeSelector가 "파드가 노드를 고르는" 방향이라면 taint/toleration은 "노드가 파드를 거르는" 반대 방향 — 둘이 합쳐져야 예를 들어 GPU 전용 노드에 일반 파드가 실수로 안 올라가게(taint) + GPU 파드만 예외로 허용(toleration)하는 구성이 완성된다. control-plane 노드에 일반 파드가 안 뜨는 것도 이 taint 덕분이다.
...
```
전략             다운타임   전환 속도       위험 분산     자원 오버헤드   난이도(K8s 네이티브 지원)
--------------  ---------  --------------  ------------  --------------  --------------------------
Recreate         있음      즉시(전체교체)   없음          없음            ★ (strategy: Recreate)
RollingUpdate    없음      점진적(순차)     낮음(순서뿐)   낮음(surge만큼)  ★ (strategy: RollingUpdate, 기본값)
Canary           없음      단계적(비율↑)    높음          중간(이중 실행)  ★★★ (Service selector 트릭 필요, 실전은 Argo Rollouts)
Blue-Green       없음      원자적(한 번에)  중간          높음(전량 이중)  ★★ (selector patch 한 줄, 이중 자원 필요)
```

- **다운타임 있는 건 Recreate뿐** — 나머지 셋은 전부 무중단이 목적
- **"위험을 얼마나 빨리 감지하고 되돌리나"** 기준으로 보면: Canary(일부만 겪으며 서서히 확인) > Blue-Green(전환 직후 이상하면 selector 한 줄로 즉시 원복) > RollingUpdate(문제 있는 버전도 이미 일부 트래픽을 받은 뒤에야 rollout이 멈춤)
- 부트캠프·포트폴리오 벤치마크에서 "배포전략(블루그린/카나리)"이 표준 항목으로 꼽히는 이유가 바로 RollingUpdate만으로는 "점진적 검증 후 확대/원복"이라는 실무 요구를 못 채우기 때문 — PF1·캡스톤에서 이 표의 아래 두 줄을 실제로 구현하는 게 목표.
...
**4장. apiserver에 요청이 도착한 뒤 etcd에 기록되기 전까지 거치는 3단계는**
→ 인증(누구인가, 인증서·토큰) → 인가(그 동작을 해도 되나, RBAC) → Admission Controller(요청이 정책에 맞나, 필요시 값 보정). 셋 다 통과해야 etcd에 기록되고 그제야 controller-manager·scheduler가 반응한다.
...
**5장. Pod가 삭제될 때 SIGKILL 전에 무슨 일이 일어나나**
→ Terminating 상태로 바뀌며 Service Endpoints에서 즉시 제외 → preStop 훅 실행 → SIGTERM 전달(앱이 스스로 정리할 시간) → terminationGracePeriodSeconds(기본 30초) 안에 안 끝나면 SIGKILL 강제 종료. 앱이 SIGTERM을 안 받으면 이 유예시간을 그냥 낭비하고 강제 종료된다.
...
**6장. Canary와 Blue-Green의 차이, 그리고 K8s Service만으로 카나리를 만드는 원리는**
→ 카나리는 일부 트래픽만 신버전에 흘려보내며 서서히 비율을 늘리는 점진적 전략(위험 분산 우선), 블루그린은 검증 끝난 새 환경으로 트래픽을 한 번에 원자적으로 전환하는 전략(전환 속도·즉시 원복 우선). K8s Service·Deployment엔 트래픽 비율 개념이 없어서, stable·canary 두 Deployment를 같은 Service selector(예: `app: web`)로 묶어 파드 개수 비율(9:1 등)로 트래픽 비율을 흉내낸다 — Service는 균등 분배만 하기 때문에 성립하는 트릭.
...
**9장. taint와 toleration을 한 문장씩으로 구분하면**
→ taint = 노드가 파드를 거부하는 표식("나는 아무나 안 받는다"). toleration = 파드가 그 거부를 견딜 수 있다는 허용 표시. nodeSelector가 파드 쪽에서 노드를 고르는 것과 반대로, taint/toleration은 노드 쪽에서 파드를 거르는 방향 — control-plane 노드에 일반 파드가 안 뜨는 것도 이 taint 때문이다.
